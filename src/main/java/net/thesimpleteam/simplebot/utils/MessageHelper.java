...
public class MessageHelper {
    ...
    public static void syntaxError(CommandEvent event, Command command, String informations) {
        ...
        if (command.getExample() == null)
            ...
            examples = Arrays.toString(Stream.of(command.getExample().split("²")).map(example -> SimpleBot.getPrefix(event.getGuild()) + command.getName() + " " + example).toArray()).replace("[", "").replace("]", "").replace(",", "");
