...
public class Listener extends ListenerAdapter {
    ...
    public static void saveConfigs() throws IOException {
        ...
        if (Main.gson.fromJson(Files.newBufferedReader(new File("config/server-config.json").toPath(), StandardCharsets.UTF_8), ServerConfig.class) == Main.getServerConfig()) return;
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent event) {
        if (Main.getServerConfig().channelMemberJoin().containsKey(event.getGuild().getId()) && event.getGuild().getTextChannelById(Main.getServerConfig().channelMemberJoin().get(event.getGuild().getId())) != null) { // Verifies if the server configured the channelMemberJoin config and if the configured channel, if all is true it verifies if the configured channel exists, if false it verifies if the configured (on Discord) system channel exists.
            try { // Try catch the InsufficientPermissionException to verifies if the bot hasn't the permission to write in the configured channel, if it catches it verifies if the owner is null.
                event.getGuild().getTextChannelById(Main.getServerConfig().channelMemberJoin().get(event.getGuild().getId())).sendMessage(new MessageBuilder(new EmbedBuilder() // Sends a memberJoin embed in the configured channel
                        ...
                        .setTitle(String.format(MessageHelper.translateMessage("success.onGuildMemberJoin.memberJoin", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), event.getMember().getEffectiveName(), event.getGuild().getName()))
                        .addField(MessageHelper.translateMessage("success.onGuildMemberJoin.member", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), event.getMember().getAsMention(), false)
                        .addField(String.format("%s %s", UnicodeCharacters.heavyPlusSign, MessageHelper.translateMessage("success.onGuildMemberJoin.newMember", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild())), String.format(MessageHelper.translateMessage("success.onGuildMemberJoin.countMember", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), event.getGuild().getMemberCount()), false)
                        .setTimestamp(Instant.now())
                        .setColor(Color.GREEN)
                        .build()).build()).queue();
            } catch (InsufficientPermissionException exception) {
                if(event.getGuild().getOwner() == null) return; // Verifies if the server's owner is null, if true it does nothing, if false it sends the channelMemberJoinHasntPermission embed in the server's owner's private channel.
                event.getGuild().getOwner().getUser().openPrivateChannel().queue(privateChannel -> // Sends the channelMemberJoinHasntPermission embed in the server's owner's private channel.
                        privateChannel.sendMessage(new MessageBuilder(new EmbedBuilder()
                                ...
                                .setTimestamp(Instant.now())
                                .setTitle(String.format("%s %s", UnicodeCharacters.crossMarkEmoji, String.format(MessageHelper.translateMessage("error.onGuildMemberJoin.channelMemberJoinHasntPermission", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), MessageHelper.getTag(event.getUser()), event.getGuild().getName())))
                                .build()).build()).queue());
            }
            return;
        } else {
            if(event.getGuild().getSystemChannel() == null) return; // Verify if the configured (on Discord) system channel exists, if true it sends a memberJoin embed in the configured (on Discord) system channel, if false it does nothing.
            ...
            try {
                ...
                if(event.getGuild().getOwner() == null) return; // Verifies if the server's owner is null, if true it does nothing, if false it sends the systemChannelHasntPermission embed in the server's owner's private channel.
                event.getGuild().getOwner().getUser().openPrivateChannel().queue(privateChannel -> // Sends the systemChannelHasntPermission embed in the server's owner's private channel.
                        privateChannel.sendMessage(new MessageBuilder(new EmbedBuilder()
                                ...
                                .setFooter(event.getGuild().getOwner().getUser().getName(), event.getGuild().getOwner().getUser().getEffectiveAvatarUrl())
                                .setTimestamp(Instant.now())
                                .setTitle(String.format("%s %s", UnicodeCharacters.crossMarkEmoji, String.format(MessageHelper.translateMessage("error.onGuildMemberJoin.systemChannelHasntPermission", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), MessageHelper.getTag(event.getUser()), event.getGuild().getName(), Main.getPrefix(event.getGuild()), Main.getPrefix(event.getGuild()))))
                                .build()).build()).queue());
            }
            /*}*/
        }
        if (Main.getServerConfig().guildJoinRole().containsKey(event.getGuild().getId())) {
            if(event.getGuild().getRoleById(Main.getServerConfig().guildJoinRole().get(event.getGuild().getId())) == null){
                if(event.getGuild().getOwner() == null) return; // Verifies if the server's owner is null, if true it does nothing, if false it sends the joinRoleNull embed in the server's owner's private channel.
                event.getGuild().getOwner().getUser().openPrivateChannel().queue(privateChannel -> // Sends the joinRoleNull embed in the server's owner's private channel.
                        privateChannel.sendMessage(new MessageBuilder(new EmbedBuilder()
                                ...
                                .setFooter(event.getGuild().getOwner().getUser().getName(), event.getGuild().getOwner().getUser().getEffectiveAvatarUrl())
                                .setTimestamp(Instant.now())
                                .setTitle(String.format("%s %s", UnicodeCharacters.crossMarkEmoji, String.format(MessageHelper.translateMessage("error.onGuildMemberJoin.joinRoleNull", event.getUser(), event.getGuild().getOwner() == null ? null : event.getGuild().getOwner().getUser(), event.getGuild()), MessageHelper.getTag(event.getUser()), event.getGuild().getName(), Main.getPrefix(event.getGuild()), Main.getPrefix(event.getGuild()))))
                                .build()).build()).queue());
                return;
            }
        ...
        Main.LOGGER.info(event.getUser().getName() + "#" + event.getUser().getDiscriminator() + " a rejoint le serveur " + event.getGuild().getName() + ".");
    }
